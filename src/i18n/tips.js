...
export const TIPS = {
  de: {
    ...
    nichtUml: `Kosten, die nicht auf den Mieter umlegbar sind: Verwaltung, Instandhaltung, Rücklagenzuführung. Als Richtwert gelten ${NU_DE.min} bis ${NU_DE.max} € je m² Wohnfläche und Monat. Der Rechner setzt die Mitte an (${NU_DE.mittel} €/m²) und passt den Wert automatisch an, wenn sich die Wohnfläche ändert. Sobald du das Feld selbst überschreibst, bleibt dein Wert stehen. Aus dem Exposé wird dieser Wert nie übernommen — die Aufteilung des Hausgelds in umlagefähig und nicht umlagefähig steht dort nur selten drin.`,
    leerstand:
    ...
    sanIstVerbrauch: `Endenergieverbrauch aus dem Energieausweis in kWh/m²a. Ist der Wert gesetzt, rechnet der Sanierungsrechner damit statt mit der groben Schätzung aus dem Baujahr — die liegt für Bauten der 90er oft 30–50 % zu hoch und lässt die Einsparung entsprechend zu günstig aussehen. Der Warmwasseranteil wird automatisch abgezogen, weil er in Ausweisen mit zentraler Warmwasserbereitung bereits enthalten ist und sonst doppelt zählt. Achtung beim Verbrauchsausweis: Der Wert hängt vom Heizverhalten der Vormieter und vom Wetter ab, ein Bedarfsausweis ist genormt. Werte außerhalb ${VERBRAUCH_GRENZEN.min}–${VERBRAUCH_GRENZEN.max} kWh/m²a werden ignoriert.`,
  ...
  en: {
    ...
    nichtUml: `Costs that cannot be passed on to the tenant: management, maintenance, reserve contributions. The common benchmark is €${NU_EN.min} to €${NU_EN.max} per m² of living area per month. The calculator applies the midpoint (€${NU_EN.mittel}/m²) and updates the value automatically whenever the living area changes. Once you override the field yourself, your value stays. This value is never taken from a property listing — listings rarely break the service charge down into recoverable and non-recoverable parts.`,
    leerstand:
    ...
    sanIstVerbrauch: `Final energy consumption from the energy certificate in kWh/m²a. When set, the renovation calculator uses this instead of the rough estimate derived from the year of construction — for 1990s buildings that estimate is often 30–50 % too high, which makes the savings look better than they are. The hot water share is deducted automatically because certificates with central hot water already include it and it would otherwise count twice. Note for consumption-based certificates: the figure depends on the previous tenants' heating habits and on the weather, whereas a demand-based certificate is standardised. Values outside ${VERBRAUCH_GRENZEN.min}–${VERBRAUCH_GRENZEN.max} kWh/m²a are ignored.`,
  ...
  tr: {
    ...
    nichtUml: `Kiracıya yansıtılamayan maliyetler: yönetim, bakım, rezerv payı. Yaygın referans değer: aylık m² başına ${NU_TR.min} – ${NU_TR.max} €. Hesaplayıcı orta değeri (${NU_TR.mittel} €/m²) kullanır ve yaşam alanı değiştiğinde değeri otomatik günceller. Alanı kendiniz değiştirdiğinizde sizin değeriniz korunur. Bu değer ilandan hiçbir zaman alınmaz — ilanlarda aidatın yansıtılabilir ve yansıtılamaz bölümü nadiren ayrılır.`,
    ...
    sanIstVerbrauch: `Enerji sertifikasındaki nihai enerji tüketimi (kWh/m²a). Değer girildiğinde yenileme hesaplayıcısı, inşaat yılına dayalı kaba tahmin yerine bunu kullanır — 90'lı yılların yapılarında bu tahmin çoğu kez %30–50 fazladır ve tasarrufu olduğundan iyi gösterir. Sıcak su payı otomatik düşülür; merkezi sıcak su sistemli sertifikalarda zaten dahildir ve aksi halde iki kez sayılır. Tüketim sertifikasında dikkat: değer önceki kiracıların ısıtma alışkanlıklarına ve hava durumuna bağlıdır, ihtiyaç sertifikası ise standartlaştırılmıştır. ${VERBRAUCH_GRENZEN.min}–${VERBRAUCH_GRENZEN.max} kWh/m²a dışındaki değerler yok sayılır.`,
  ...
  zh: {
    ...
    nichtUml: `不能转嫁给租户的费用：管理费、维护费、储备金提存。通用参考值为每平方米居住面积每月 ${NU_EN.min} 至 ${NU_EN.max} 欧元。计算器采用中间值（${NU_EN.mittel} 欧元/m²），并在居住面积变化时自动更新。一旦您手动修改该字段，将保留您输入的值。此数值绝不会从房源资料中提取——房源资料很少将物业费拆分为可转嫁和不可转嫁部分。`,
    ...
    sanIstVerbrauch: `能源证书中的终端能耗（kWh/m²a）。填入后，改造计算器将采用该数值，而不是依据建筑年份的粗略估算——对上世纪90年代的建筑，该估算常偏高30–50%，会让节能效果显得过于乐观。热水部分会自动扣除，因为集中供热水的证书已包含该部分，否则会重复计算。使用消耗型证书时请注意：数值取决于前住户的供暖习惯和天气，而需求型证书是标准化的。超出 ${VERBRAUCH_GRENZEN.min}–${VERBRAUCH_GRENZEN.max} kWh/m²a 的数值将被忽略。`,
  ...
  hi: {
    ...
    nichtUml: `वे खर्च जो किरायेदार पर नहीं डाले जा सकते: प्रबंधन, रखरखाव, आरक्षित निधि अंशदान। सामान्य मानक मूल्य: प्रति m² रहने के क्षेत्र प्रति माह €${NU_EN.min} से €${NU_EN.max}। कैलकुलेटर मध्य मान (€${NU_EN.mittel}/m²) लेता है और रहने का क्षेत्र बदलने पर मान स्वतः अपडेट करता है। जैसे ही आप फ़ील्ड स्वयं बदलते हैं, आपका मान बना रहता है। यह मान कभी भी प्रॉपर्टी लिस्टिंग से नहीं लिया जाता — लिस्टिंग में मेंटेनेंस शुल्क को शायद ही कभी वसूली-योग्य और गैर-वसूली-योग्य भागों में बाँटा जाता है।`,
    ...
    sanIstVerbrauch: `ऊर्जा प्रमाणपत्र से अंतिम ऊर्जा खपत (kWh/m²a)। मान भरने पर नवीनीकरण कैलकुलेटर निर्माण वर्ष से लगाए गए मोटे अनुमान के बजाय इसका उपयोग करता है — 1990 के दशक की इमारतों के लिए वह अनुमान अक्सर 30–50 % अधिक होता है और बचत को वास्तविकता से बेहतर दिखाता है। गर्म पानी का हिस्सा स्वतः घटा दिया जाता है, क्योंकि केंद्रीय गर्म पानी वाले प्रमाणपत्रों में वह पहले से शामिल होता है और अन्यथा दो बार गिना जाता। खपत-आधारित प्रमाणपत्र में ध्यान दें: मान पिछले किरायेदारों की हीटिंग आदतों और मौसम पर निर्भर करता है, जबकि माँग-आधारित प्रमाणपत्र मानकीकृत होता है। ${VERBRAUCH_GRENZEN.min}–${VERBRAUCH_GRENZEN.max} kWh/m²a से बाहर के मान अनदेखे किए जाते हैं।`,
