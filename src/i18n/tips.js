...
export const TIPS = {
  de: {
    ...
    kfwAktiv: `Förderkredit der KfW als zweites Darlehen neben dem Bankdarlehen — eigener Zins, eigene Laufzeit, eigene Tilgung. Der günstigere KfW-Zins senkt den Mischzins der Gesamtfinanzierung. Konditionen Stand ${KFW_KREDIT.stand}; sie werden erst bei der Zusage festgeschrieben.`,
    ...
    kfwZins: `Effektivzins des Förderkredits. Leer lassen übernimmt den Richtwert aus den hinterlegten Konditionen (${Z_DE(KFN.zins)} % für ${KFN.nr}, ${Z_DE(WEP.zins)} % für ${WEP.nr}, Stand ${KFW_KREDIT.stand}). Der echte Zins hängt von Laufzeit, Zinsbindung und tilgungsfreien Jahren ab und steht erst in der Zusage deiner Hausbank.`,
  ...
  en: {
    ...
    kfwAktiv: `A KfW subsidised loan running as a second loan alongside the bank loan — its own rate, term and repayment. The lower KfW rate reduces the blended rate of the overall financing. Conditions as of ${KFW_KREDIT.stand}; they are only fixed when the loan is approved.`,
    ...
    kfwZins: `Effective rate of the subsidised loan. Leaving it empty applies the stored benchmark (${Z_EN(KFN.zins)} % for ${KFN.nr}, ${Z_EN(WEP.zins)} % for ${WEP.nr}, as of ${KFW_KREDIT.stand}). The real rate depends on term, fixed-rate period and grace years and is only stated in your bank's approval.`,
  ...
  tr: {
    ...
    kfwAktiv: `Banka kredisinin yanında ikinci kredi olarak işleyen KfW teşvik kredisi — kendi faizi, vadesi ve anapara ödemesi vardır. Daha düşük KfW faizi, toplam finansmanın karma faizini aşağı çeker. Koşullar ${KFW_KREDIT.stand} itibariyledir; ancak onay aşamasında kesinleşir.`,
    ...
    kfwZins: `Teşvik kredisinin efektif faizi. Boş bırakılırsa kayıtlı referans değer kullanılır (${KFN.nr} için %${Z_TR(KFN.zins)}, ${WEP.nr} için %${Z_TR(WEP.zins)}, ${KFW_KREDIT.stand} itibariyle). Gerçek faiz vadeye, faiz sabitleme süresine ve anapara ödemesiz yıllara bağlıdır ve yalnızca bankanızın onayında yer alır.`,
  ...
  zh: {
    ...
    kfwAktiv: `KfW 政策性贷款作为银行贷款之外的第二笔贷款——有自己的利率、期限和还款。较低的 KfW 利率会拉低整体融资的混合利率。条件为 ${KFW_KREDIT.stand} 时点数据；实际条件在批贷时才最终确定。`,
    ...
    kfwZins: `政策性贷款的实际利率。留空则采用内置参考值（${KFN.nr} 为 ${Z_EN(KFN.zins)}%，${WEP.nr} 为 ${Z_EN(WEP.zins)}%，${KFW_KREDIT.stand} 时点）。真实利率取决于期限、利率锁定期和免还本年数，只有在你的开户行批贷时才会写明。`,
  ...
  hi: {
    ...
    kfwAktiv: `बैंक ऋण के साथ दूसरे ऋण के रूप में चलने वाला KfW सब्सिडी ऋण — अपनी ब्याज दर, अवधि और चुकौती के साथ। कम KfW दर कुल वित्तपोषण की मिश्रित दर को घटाती है। शर्तें ${KFW_KREDIT.stand} तक; वे स्वीकृति के समय ही अंतिम रूप से तय होती हैं।`,
    ...
    kfwZins: `सब्सिडी ऋण की प्रभावी दर। खाली छोड़ने पर संग्रहीत संदर्भ मान लागू होता है (${KFN.nr} हेतु ${Z_EN(KFN.zins)}%, ${WEP.nr} हेतु ${Z_EN(WEP.zins)}%, ${KFW_KREDIT.stand} तक)। वास्तविक दर अवधि, ब्याज-स्थिरता अवधि और चुकौती-मुक्त वर्षों पर निर्भर करती है और केवल आपके बैंक की स्वीकृति में दर्ज होती है।`,
