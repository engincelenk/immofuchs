// i18n fuer Expose-/Screenshot-Upload (Spec Abschnitt 8, i18n-Absatz).
// Eigener Modul statt assistant.js, weil hier ~30 Feldbezeichner dazukommen -
// gleiches Muster wie steuerTrick.js/vorfaelligkeit.js.
//
// tr/zh/hi sind Best-Effort-Uebersetzungen, vor Live-Schaltung von
// Muttersprachlern gegenlesen lassen (wie im Rest des Projekts).
//
// Die Schluessel unter `felder` entsprechen exakt den Feldnamen des
// Worker-Schemas (Spec 5) - die bleiben sprachunabhaengig Deutsch, weil sie
// ein technischer Vertrag zwischen Client und Worker sind.

export const EXPOSE_T = {
  de: {
    attachAria: "Exposé oder Screenshots hochladen",
    chipUpload: "📄 Exposé hochladen",
    dateiWaehlen: "Exposé oder Screenshots wählen",
    entfernenAria: "Bild entfernen",
    bilderGewaehlt: "{n} Datei(en) ausgewählt",
    auswertenBtn: "Exposé auswerten",
    consentText:
      "Deine Bilder und PDFs werden zur Analyse kurzzeitig an unseren Server übertragen und nicht dauerhaft gespeichert.",
    consentOk: "Verstanden",
    phaseUpload: "{n} von {m} Bildern vorbereitet",
    phaseExtract: "Ich lese das Exposé",
    phaseExtractSchritte: ["Objektdaten", "Kosten", "Energie", "Lage"],
    resultTitel: "{gefunden} von {gesamt} Feldern gefunden · {pruefen} zu prüfen",
    roentgenTitel: "Exposé-Röntgen: Risiken & Auffälligkeiten",
    risikoCode_sanierungsstau: "Sanierungsstau",
    risikoCode_energieklasse_schlecht: "Schlechte Energieklasse",
    risikoCode_modernisierung_angekuendigt: "Modernisierung angekündigt",
    risikoCode_erbpacht: "Erbpacht",
    risikoCode_denkmalschutz: "Denkmalschutz",
    risikoCode_rechtsstreit: "Rechtsstreit",
    risikoCode_miete_unter_markt: "Miete unter Marktniveau",
    risikoCode_lange_standzeit: "Lange Inseratsdauer",
    risikoCode_leerstand_lang: "Längerer Leerstand",
    risikoCode_sonstiges: "Sonstige Auffälligkeit",
    gruppeObjekt: "Objekt",
    gruppeAusstattung: "Ausstattung",
    gruppeEnergie: "Energie",
    gruppeKosten: "Kosten",
    gruppeKontext: "Beschreibung",
    uebernehmen: "Übernehmen",
    allesMarkieren: "Alles markieren",
    auswahlAufheben: "Auswahl aufheben",
    uebernommen: "{n} Werte übernommen",
    zumRechnerBtn: "Weiter zum Rechner ▸",
    keineUebernahme: "Nichts ausgewählt",
    aktuell: "aktuell: {wert}",
    nurInfo: "nur zur Info",
    nichtGefunden: "nicht gefunden",
    ja: "ja",
    nein: "nein",
    statusSicher: "sicher gelesen",
    statusUnsicher: "unsicher — bitte prüfen",
    statusPruefen: "bitte prüfen",
    statusNichtGefunden: "nicht gefunden",
    fehlerDienst: "Ich konnte das Exposé gerade nicht auswerten. Bitte versuch es gleich noch einmal.",
    fehlerUnlesbar:
      "Aus dem Exposé konnte ich nichts auslesen. Bei Fotos hilft eine höhere Auflösung, bei PDFs eines mit echtem Text statt eingescannter Seiten.",
    fehlerLimit: "Dein Exposé-Kontingent ist gerade aufgebraucht — bei Free gibt es am Monatsersten neue, bei Pro morgen.",
    fehlerZugang: "Für den Exposé-Scan brauchst du ein Konto — sieben Tage kostenlos testen.",
    fehlerDeaktiviert: "Die Exposé-Auswertung ist gerade nicht verfügbar.",
    fehlerOffline: "Dafür brauche ich kurz Internet.",
    fehlerZuVieleBilder: "Maximal 15 Bilder pro Exposé.",
    fehlerPdfZuGross: "Das PDF ist zu groß (max. 25 MB).",
    fehlerPdfZuVieleSeiten: "Das PDF hat zu viele Seiten (max. 20).",
    fehlerNurEinPdf: "Nur ein PDF pro Exposé.",
    fehlerTypNichtErlaubt: "Nur Bilder (JPG, PNG, WebP) oder ein PDF.",
    fehlerZuGrossGesamt: "Die Auswahl ist insgesamt zu groß. Bitte weniger Bilder.",
    fehlerLesen: "Eine Datei konnte nicht gelesen werden.",
    disclaimer:
      "Diese Werte stammen aus der Anzeige des Anbieters — nicht geprüft. Wohnfläche und Energiewerte weichen in der Praxis häufig ab; bei ernsthaftem Interesse vor Ort nachmessen bzw. Energieausweis im Original einsehen.",
    // Was Finn im Chat sagt, waehrend und nach dem Upload. Zweck: der Nutzer
    // soll wissen, was gerade passiert und was gleich auf ihn zukommt -
    // sonst ist der Upload eine Blackbox mit Fortschrittsbalken.
    // Ton: Finn nimmt Arbeit ab (Abtippen) und bereitet auf den Termin vor.
    // Neutral gegenueber dem Anbieter (GR-02), keine Kaufempfehlung.
    finnSagt: {
      upload:
        "Alles klar, ich schaue es mir an. Ich lese das Exposé durch und ziehe die Werte heraus, die deine Rechner brauchen — Kaufpreis, Wohnfläche, Hausgeld, Energiedaten. Das Abtippen sparst du dir.",
      handout:
        "Fertig. Ich habe das Exposé gleich noch gegengelesen und dabei {n} Punkte gefunden, die vor einer Zusage geklärt gehören. Unten liegt dein Handout für die Besichtigung: der echte Preis, die offenen Punkte und die Fragen, die beim Termin wirklich zählen. Ausdrucken, mitnehmen, abhaken.",
      handoutLeer:
        "Fertig. Beim Gegenlesen ist mir nichts Größeres aufgefallen — gutes Zeichen. Ein Handout für die Besichtigung habe ich dir trotzdem vorbereitet: der echte Preis auf einen Blick und die Fragen, die das Exposé offenlässt. Damit gehst du vorbereitet in den Termin.",
    },
    handout: {
      titel: "Besichtigungs-Handout",
      oeffnen: "Handout öffnen",
      schliessen: "Handout schließen",
      groessterFund: "größter Fund {betrag}",
      keinFund: "keine Auffälligkeiten",
      // GR-01: muss im Panel stehen, nicht nur im PDF.
      automatisiert: "Automatisiert erkannt — bitte vor einer Verhandlung selbst prüfen.",
      findings: "Die wichtigsten Punkte",
      weitere: "+{n} weitere nicht angezeigt",
      quellen: "Fundstellen: {quellen}",
      schwereCritical: "kritisch",
      schwereWarning: "prüfen",
      schwereInfo: "Info",
      preis: "Der echte Preis",
      beworben: "Beworben",
      real: "Real",
      bekannt: "Bereits bekannt",
      offen: "Noch zu klären",
      vorOrt: "Vor Ort prüfen",
      leer: "Keine wesentlichen Auffälligkeiten gefunden. Die offenen Fragen unten lohnen den Termin trotzdem.",
      alleWaehlen: "Alle auswählen",
      alleAbwaehlen: "Auswahl aufheben",
      pdfBtn: "PDF für die Besichtigung",
      pdfLeer: "Bitte mindestens eine Frage auswählen.",
      // PDF-Texte
      pdfDisclaimer: "Keine Rechts- oder Anlageberatung",
      pdfFooter: "Erstellt mit Finn · immofuchs.info",
      pdfVerdict: "Finn-Einschätzung {score} / 5",
    },
    felder: {
      titel: "Titel",
      objektart: "Objektart",
      kaufpreis: "Kaufpreis",
      stellplatz_kaufpreis: "Stellplatz-Kaufpreis",
      kaufpreis_pro_qm: "Preis pro m²",
      zimmer: "Zimmer",
      wohnflaeche: "Wohnfläche",
      plz: "PLZ",
      ort: "Ort",
      stockwerk: "Stockwerk",
      baujahr: "Baujahr",
      balkon_terrasse: "Balkon/Terrasse",
      einbaukueche: "Einbauküche",
      stellplatz: "Stellplatz",
      keller: "Keller",
      barrierefrei: "Barrierefrei",
      heizungsart: "Heizungsart",
      energieausweistyp: "Energieausweis",
      energietraeger: "Energieträger",
      endenergiebedarf: "Endenergiebedarf",
      energieeffizienzklasse: "Effizienzklasse",
      hausgeld: "Hausgeld",
      provision_kaeufer_prozent: "Käuferprovision",
      kaufnebenkosten: "Kaufnebenkosten",
      gesamtkosten: "Gesamtkosten",
      kaltmiete: "Kaltmiete",
      nebenkosten_miete: "Nebenkosten",
      objektbeschreibung: "Objektbeschreibung",
      lagebeschreibung: "Lage",
      nutzflaeche: "Nutzfläche",
      strasse: "Straße",
      hausnummer: "Hausnummer",
      zustand: "Zustand",
      wohneinheiten: "Wohneinheiten im Haus",
      vermietet: "Vermietet",
      vermietet_seit: "Vermietet seit",
      stellplatz_anzahl: "Anzahl Stellplätze",
      baujahr_waermeerzeuger: "Baujahr Wärmeerzeuger",
      hausgeld_nicht_umlagefaehig: "Hausgeld nicht umlagefähig",
      ruecklage_monatlich: "Instandhaltungsrücklage",
    },
  },

  en: {
    attachAria: "Upload listing or screenshots",
    chipUpload: "📄 Upload listing",
    dateiWaehlen: "Choose listing or screenshots",
    entfernenAria: "Remove image",
    bilderGewaehlt: "{n} file(s) selected",
    auswertenBtn: "Analyse listing",
    consentText:
      "Your images and PDFs are sent to our server briefly for analysis and are not stored permanently.",
    consentOk: "Got it",
    phaseUpload: "{n} of {m} images prepared",
    phaseExtract: "Reading the listing",
    phaseExtractSchritte: ["Property data", "Costs", "Energy", "Location"],
    resultTitel: "{gefunden} of {gesamt} fields found · {pruefen} to check",
    roentgenTitel: "Property X-ray: risks & red flags",
    risikoCode_sanierungsstau: "Deferred maintenance",
    risikoCode_energieklasse_schlecht: "Poor energy rating",
    risikoCode_modernisierung_angekuendigt: "Modernization announced",
    risikoCode_erbpacht: "Leasehold (Erbpacht)",
    risikoCode_denkmalschutz: "Heritage protection",
    risikoCode_rechtsstreit: "Legal dispute",
    risikoCode_miete_unter_markt: "Rent below market",
    risikoCode_lange_standzeit: "Long time on market",
    risikoCode_leerstand_lang: "Extended vacancy",
    risikoCode_sonstiges: "Other red flag",
    gruppeObjekt: "Property",
    gruppeAusstattung: "Features",
    gruppeEnergie: "Energy",
    gruppeKosten: "Costs",
    gruppeKontext: "Description",
    uebernehmen: "Apply",
    allesMarkieren: "Select all",
    auswahlAufheben: "Clear selection",
    uebernommen: "{n} values applied",
    zumRechnerBtn: "Go to calculator ▸",
    keineUebernahme: "Nothing selected",
    aktuell: "current: {wert}",
    nurInfo: "info only",
    nichtGefunden: "not found",
    ja: "yes",
    nein: "no",
    statusSicher: "read reliably",
    statusUnsicher: "uncertain — please check",
    statusPruefen: "please check",
    statusNichtGefunden: "not found",
    fehlerDienst: "I couldn't analyse the listing just now. Please try again in a moment.",
    fehlerUnlesbar:
      "I couldn't read anything from the listing. For photos, a higher resolution helps; for PDFs, one with real text rather than scanned pages.",
    fehlerLimit: "Your listing quota is used up for now — Free gets new ones on the 1st, Pro tomorrow.",
    fehlerZugang: "The listing scan needs an account — seven days free.",
    fehlerDeaktiviert: "Listing analysis is currently unavailable.",
    fehlerOffline: "I need an internet connection for this.",
    fehlerZuVieleBilder: "Maximum 15 images per listing.",
    fehlerPdfZuGross: "The PDF is too large (max. 25 MB).",
    fehlerPdfZuVieleSeiten: "The PDF has too many pages (max. 20).",
    fehlerNurEinPdf: "Only one PDF per listing.",
    fehlerTypNichtErlaubt: "Images (JPG, PNG, WebP) or one PDF only.",
    fehlerZuGrossGesamt: "The selection is too large overall. Please use fewer images.",
    fehlerLesen: "One of the files could not be read.",
    disclaimer:
      "These values come from the seller's listing — unverified. Floor area and energy figures often differ in practice; if you're seriously interested, measure on site and view the original energy certificate.",
    // Hinweis: die Fragen des Katalogs selbst bleiben in allen Sprachen
    // deutsch - sie haengen an deutschem Immobilienrecht (§ 577a BGB, GEG,
    // WoFlV, WEG). Uebersetzt waeren sie inhaltlich irrefuehrend (Spec v2,
    // Abschnitt 7, i18n-Absatz). Nur die Rahmen-Beschriftungen sind lokalisiert.
    finnSagt: {
      upload:
        "Right, let me take a look. I'll read through the listing and pull out the figures your calculators need — purchase price, floor area, service charge, energy data. Saves you the typing.",
      handout:
        "Done. I also cross-read the listing and found {n} points worth clarifying before you commit. Your viewing handout is below: the real price, the open points and the questions that actually matter on the day. Print it, take it, tick it off.",
      handoutLeer:
        "Done. Nothing major stood out on the cross-read — a good sign. I've prepared a viewing handout anyway: the real price at a glance and the questions the listing leaves open. So you walk in prepared.",
    },
    handout: {
      titel: "Viewing handout",
      oeffnen: "Open handout",
      schliessen: "Close handout",
      groessterFund: "largest finding {betrag}",
      keinFund: "nothing unusual",
      automatisiert: "Detected automatically — please verify yourself before negotiating.",
      findings: "The key points",
      weitere: "+{n} more not shown",
      quellen: "Found in: {quellen}",
      schwereCritical: "critical",
      schwereWarning: "check",
      schwereInfo: "info",
      preis: "The real price",
      beworben: "Advertised",
      real: "Real",
      bekannt: "Already known",
      offen: "Still to clarify",
      vorOrt: "Check on site",
      leer: "Nothing significant found. The open questions below are still worth raising at the viewing.",
      alleWaehlen: "Select all",
      alleAbwaehlen: "Clear selection",
      pdfBtn: "PDF for the viewing",
      pdfLeer: "Please select at least one question.",
      pdfDisclaimer: "Not legal or investment advice",
      pdfFooter: "Created with Finn · immofuchs.info",
      pdfVerdict: "Finn assessment {score} / 5",
    },
    felder: {
      titel: "Title",
      objektart: "Property type",
      kaufpreis: "Purchase price",
      stellplatz_kaufpreis: "Parking space price",
      kaufpreis_pro_qm: "Price per m²",
      zimmer: "Rooms",
      wohnflaeche: "Floor area",
      plz: "Postcode",
      ort: "Town",
      stockwerk: "Floor",
      baujahr: "Year built",
      balkon_terrasse: "Balcony/terrace",
      einbaukueche: "Fitted kitchen",
      stellplatz: "Parking space",
      keller: "Cellar",
      barrierefrei: "Step-free access",
      heizungsart: "Heating type",
      energieausweistyp: "Energy certificate",
        nutzflaeche: "Usable area",
      strasse: "Street",
      hausnummer: "House number",
      zustand: "Condition",
      wohneinheiten: "Units in building",
      vermietet: "Currently rented",
      vermietet_seit: "Rented since",
      stellplatz_anzahl: "Parking spaces",
      baujahr_waermeerzeuger: "Heating system year",
      hausgeld_nicht_umlagefaehig: "Non-recoverable service charge",
      ruecklage_monatlich: "Maintenance reserve",
    energietraeger: "Energy source",
      endenergiebedarf: "Final energy demand",
      energieeffizienzklasse: "Efficiency class",
      hausgeld: "Service charge",
      provision_kaeufer_prozent: "Buyer's commission",
      kaufnebenkosten: "Purchase costs",
      gesamtkosten: "Total costs",
      kaltmiete: "Base rent",
      nebenkosten_miete: "Utilities",
      objektbeschreibung: "Property description",
      lagebeschreibung: "Location",
    },
  },

  tr: {
    attachAria: "İlan veya ekran görüntüsü yükle",
    chipUpload: "📄 İlanı yükle",
    dateiWaehlen: "İlan veya ekran görüntüsü seç",
    entfernenAria: "Görseli kaldır",
    bilderGewaehlt: "{n} dosya seçildi",
    auswertenBtn: "İlanı değerlendir",
    consentText:
      "Görsellerin ve PDF'lerin analiz için kısa süreliğine sunucumuza aktarılır ve kalıcı olarak saklanmaz.",
    consentOk: "Anladım",
    phaseUpload: "{m} görselden {n} tanesi hazırlandı",
    phaseExtract: "İlanı okuyorum",
    phaseExtractSchritte: ["Taşınmaz verileri", "Maliyetler", "Enerji", "Konum"],
    resultTitel: "{gesamt} alandan {gefunden} tanesi bulundu · {pruefen} kontrol edilmeli",
    roentgenTitel: "Emlak röntgeni: riskler ve dikkat çeken noktalar",
    risikoCode_sanierungsstau: "Birikmiş bakım eksikliği",
    risikoCode_energieklasse_schlecht: "Düşük enerji sınıfı",
    risikoCode_modernisierung_angekuendigt: "Modernizasyon duyurulmuş",
    risikoCode_erbpacht: "İntifa hakkı (Erbpacht)",
    risikoCode_denkmalschutz: "Anıt koruması",
    risikoCode_rechtsstreit: "Hukuki uyuşmazlık",
    risikoCode_miete_unter_markt: "Piyasa altı kira",
    risikoCode_lange_standzeit: "Uzun ilan süresi",
    risikoCode_leerstand_lang: "Uzun süreli boşluk",
    risikoCode_sonstiges: "Diğer dikkat çeken nokta",
    gruppeObjekt: "Taşınmaz",
    gruppeAusstattung: "Donanım",
    gruppeEnergie: "Enerji",
    gruppeKosten: "Maliyetler",
    gruppeKontext: "Açıklama",
    uebernehmen: "Aktar",
    allesMarkieren: "Tümünü seç",
    auswahlAufheben: "Seçimi kaldır",
    uebernommen: "{n} değer aktarıldı",
    zumRechnerBtn: "Hesaplayıcıya git ▸",
    keineUebernahme: "Hiçbir şey seçilmedi",
    aktuell: "mevcut: {wert}",
    nurInfo: "yalnızca bilgi",
    nichtGefunden: "bulunamadı",
    ja: "evet",
    nein: "hayır",
    statusSicher: "güvenilir okundu",
    statusUnsicher: "belirsiz — lütfen kontrol et",
    statusPruefen: "lütfen kontrol et",
    statusNichtGefunden: "bulunamadı",
    fehlerDienst: "İlanı şu anda değerlendiremedim. Lütfen birazdan tekrar dene.",
    fehlerUnlesbar:
      "İlandan hiçbir şey okuyamadım. Fotoğraflarda daha yüksek çözünürlük, PDF'lerde ise taranmış sayfalar yerine gerçek metin içeren bir dosya yardımcı olur.",
    fehlerLimit: "İlan kontenjanın şimdilik doldu — Free'de ayın 1'inde, Pro'da yarın yenileniyor.",
    fehlerZugang: "İlan taraması için bir hesap gerekiyor — yedi gün ücretsiz.",
    fehlerDeaktiviert: "İlan değerlendirmesi şu anda kullanılamıyor.",
    fehlerOffline: "Bunun için kısa süreliğine internete ihtiyacım var.",
    fehlerZuVieleBilder: "İlan başına en fazla 15 görsel.",
    fehlerPdfZuGross: "PDF çok büyük (en fazla 25 MB).",
    fehlerPdfZuVieleSeiten: "PDF çok fazla sayfa içeriyor (en fazla 20).",
    fehlerNurEinPdf: "İlan başına yalnızca bir PDF.",
    fehlerTypNichtErlaubt: "Yalnızca görsel (JPG, PNG, WebP) veya bir PDF.",
    fehlerZuGrossGesamt: "Seçim toplamda çok büyük. Lütfen daha az görsel.",
    fehlerLesen: "Dosyalardan biri okunamadı.",
    disclaimer:
      "Bu değerler satıcının ilanından alınmıştır — doğrulanmamıştır. Yaşam alanı ve enerji değerleri pratikte sıklıkla farklılık gösterir; ciddi ilgi durumunda yerinde ölçüm yapın ve enerji belgesinin aslını görün.",
    finnSagt: {
      upload:
        "Tamam, bir bakayım. İlanı okuyup hesaplayıcıların ihtiyaç duyduğu değerleri çıkaracağım — satış fiyatı, yaşam alanı, aidat, enerji verileri. Elle yazma derdinden kurtulursun.",
      handout:
        "Hazır. İlanı ayrıca karşılaştırmalı okudum ve söz vermeden önce netleştirilmesi gereken {n} nokta buldum. Görüşme notun aşağıda: gerçek fiyat, açık kalan noktalar ve randevuda asıl önemli olan sorular. Yazdır, yanına al, tek tek işaretle.",
      handoutLeer:
        "Hazır. Karşılaştırmalı okumada dikkat çeken büyük bir şey çıkmadı — iyi işaret. Yine de görüşme için bir not hazırladım: tek bakışta gerçek fiyat ve ilanın yanıtsız bıraktığı sorular. Böylece randevuya hazırlıklı gidersin.",
    },
    handout: {
      titel: "Görüşme notu",
      oeffnen: "Notu aç",
      schliessen: "Notu kapat",
      groessterFund: "en büyük bulgu {betrag}",
      keinFund: "dikkat çeken bir şey yok",
      automatisiert: "Otomatik tespit edildi — pazarlıktan önce lütfen kendiniz doğrulayın.",
      findings: "En önemli noktalar",
      weitere: "+{n} tane daha gösterilmiyor",
      quellen: "Bulunduğu yer: {quellen}",
      schwereCritical: "kritik",
      schwereWarning: "kontrol et",
      schwereInfo: "bilgi",
      preis: "Gerçek fiyat",
      beworben: "İlan edilen",
      real: "Gerçek",
      bekannt: "Zaten bilinen",
      offen: "Netleştirilecek",
      vorOrt: "Yerinde kontrol et",
      leer: "Önemli bir bulgu yok. Aşağıdaki açık sorular yine de görüşmede sorulmaya değer.",
      alleWaehlen: "Tümünü seç",
      alleAbwaehlen: "Seçimi kaldır",
      pdfBtn: "Görüşme için PDF",
      pdfLeer: "Lütfen en az bir soru seçin.",
      pdfDisclaimer: "Hukuki veya yatırım danışmanlığı değildir",
      pdfFooter: "Finn ile oluşturuldu · immofuchs.info",
      pdfVerdict: "Finn değerlendirmesi {score} / 5",
    },
    felder: {
      titel: "Başlık",
      objektart: "Taşınmaz türü",
      kaufpreis: "Satış fiyatı",
      stellplatz_kaufpreis: "Otopark satış fiyatı",
      kaufpreis_pro_qm: "m² fiyatı",
      zimmer: "Oda",
      wohnflaeche: "Yaşam alanı",
      plz: "Posta kodu",
      ort: "Şehir",
      stockwerk: "Kat",
      baujahr: "Yapım yılı",
      balkon_terrasse: "Balkon/teras",
      einbaukueche: "Ankastre mutfak",
      stellplatz: "Otopark",
      keller: "Bodrum",
      barrierefrei: "Engelsiz erişim",
      heizungsart: "Isıtma türü",
      energieausweistyp: "Enerji belgesi",
      energietraeger: "Enerji kaynağı",
      endenergiebedarf: "Nihai enerji ihtiyacı",
      energieeffizienzklasse: "Verimlilik sınıfı",
      hausgeld: "Aidat",
      provision_kaeufer_prozent: "Alıcı komisyonu",
      kaufnebenkosten: "Alım yan maliyetleri",
      gesamtkosten: "Toplam maliyet",
      kaltmiete: "Çıplak kira",
      nebenkosten_miete: "Yan giderler",
      objektbeschreibung: "Taşınmaz açıklaması",
      lagebeschreibung: "Konum",
      nutzflaeche: "Kullanım alanı",
      strasse: "Sokak",
      hausnummer: "Kapı numarası",
      zustand: "Durum",
      wohneinheiten: "Binadaki daire sayısı",
      vermietet: "Kirada",
      vermietet_seit: "Kira başlangıcı",
      stellplatz_anzahl: "Otopark sayısı",
      baujahr_waermeerzeuger: "Isıtma sistemi yılı",
      hausgeld_nicht_umlagefaehig: "Yansıtılamayan aidat",
      ruecklage_monatlich: "Bakım rezervi",
    },
  },

  zh: {
    attachAria: "上传房源或截图",
    chipUpload: "📄 上传房源资料",
    dateiWaehlen: "选择房源或截图",
    entfernenAria: "移除图片",
    bilderGewaehlt: "已选择 {n} 个文件",
    auswertenBtn: "解析房源",
    consentText: "你的图片和 PDF 会短暂传输到我们的服务器进行分析，不会长期保存。",
    consentOk: "明白了",
    phaseUpload: "已准备 {m} 张图片中的 {n} 张",
    phaseExtract: "正在读取房源",
    phaseExtractSchritte: ["房产数据", "费用", "能源", "位置"],
    resultTitel: "在 {gesamt} 个字段中找到 {gefunden} 个 · {pruefen} 个需核对",
    roentgenTitel: "房产透视:风险与注意事项",
    risikoCode_sanierungsstau: "维修积压",
    risikoCode_energieklasse_schlecht: "能效等级差",
    risikoCode_modernisierung_angekuendigt: "已宣布现代化改造",
    risikoCode_erbpacht: "地上权(非完全产权)",
    risikoCode_denkmalschutz: "文物保护",
    risikoCode_rechtsstreit: "法律纠纷",
    risikoCode_miete_unter_markt: "租金低于市场水平",
    risikoCode_lange_standzeit: "挂牌时间长",
    risikoCode_leerstand_lang: "长期空置",
    risikoCode_sonstiges: "其他注意事项",
    gruppeObjekt: "房产",
    gruppeAusstattung: "配置",
    gruppeEnergie: "能源",
    gruppeKosten: "费用",
    gruppeKontext: "描述",
    uebernehmen: "应用",
    allesMarkieren: "全选",
    auswahlAufheben: "取消全选",
    uebernommen: "已应用 {n} 个数值",
    zumRechnerBtn: "前往计算器 ▸",
    keineUebernahme: "未选择任何项",
    aktuell: "当前：{wert}",
    nurInfo: "仅供参考",
    nichtGefunden: "未找到",
    ja: "是",
    nein: "否",
    statusSicher: "读取可靠",
    statusUnsicher: "不确定 — 请核对",
    statusPruefen: "请核对",
    statusNichtGefunden: "未找到",
    fehlerDienst: "刚才没能解析这个房源，请稍后再试一次。",
    fehlerUnlesbar:
      "没能从房源中读出任何内容。照片请提供更高分辨率；PDF 请使用含真实文字的文件，而非扫描页面。",
    fehlerLimit: "房源解析额度暂时用完了 — 免费版每月 1 日恢复，Pro 版明天恢复。",
    fehlerZugang: "房源解析需要账户 — 七天免费试用。",
    fehlerDeaktiviert: "房源解析目前不可用。",
    fehlerOffline: "这需要联网。",
    fehlerZuVieleBilder: "每个房源最多 15 张图片。",
    fehlerPdfZuGross: "PDF 太大（最多 25 MB）。",
    fehlerPdfZuVieleSeiten: "PDF 页数过多（最多 20 页）。",
    fehlerNurEinPdf: "每个房源仅限一个 PDF。",
    fehlerTypNichtErlaubt: "仅支持图片（JPG、PNG、WebP）或一个 PDF。",
    fehlerZuGrossGesamt: "所选文件总体积过大，请减少图片数量。",
    fehlerLesen: "有一个文件无法读取。",
    disclaimer:
      "这些数据来自卖方房源信息 — 未经核实。实际居住面积和能耗数据常有出入；如认真考虑，请实地测量并查看能源证书原件。",
    finnSagt: {
      upload:
        "好，我来看看。我会通读这份房源资料，把计算器需要的数据提取出来 — 购买价格、居住面积、物业费、能耗数据。省得你手动输入。",
      handout:
        "完成。我还对照通读了一遍，发现 {n} 处在你答应之前值得先弄清楚的地方。看房清单在下面：真实价格、待澄清的要点，以及看房当天真正该问的问题。打印带上，逐项核对。",
      handoutLeer:
        "完成。对照通读下来没有发现大问题 — 这是好迹象。我还是为你准备了看房清单：一眼看到真实价格，以及房源资料没有回答的问题。这样你看房时心里有数。",
    },
    handout: {
      titel: "看房清单",
      oeffnen: "打开清单",
      schliessen: "关闭清单",
      groessterFund: "最大发现 {betrag}",
      keinFund: "未发现异常",
      automatisiert: "由系统自动识别 — 议价前请自行核实。",
      findings: "关键要点",
      weitere: "还有 {n} 项未显示",
      quellen: "出处：{quellen}",
      schwereCritical: "严重",
      schwereWarning: "需核对",
      schwereInfo: "提示",
      preis: "真实价格",
      beworben: "宣传值",
      real: "实际值",
      bekannt: "已知信息",
      offen: "待澄清",
      vorOrt: "现场查看",
      leer: "未发现明显问题。下方的待澄清问题仍值得在看房时提出。",
      alleWaehlen: "全选",
      alleAbwaehlen: "取消全选",
      pdfBtn: "看房用 PDF",
      pdfLeer: "请至少选择一个问题。",
      pdfDisclaimer: "不构成法律或投资建议",
      pdfFooter: "由 Finn 生成 · immofuchs.info",
      pdfVerdict: "Finn 评估 {score} / 5",
    },
    felder: {
      titel: "标题",
      objektart: "房产类型",
      kaufpreis: "购买价格",
      stellplatz_kaufpreis: "车位价格",
      kaufpreis_pro_qm: "每平米价格",
      zimmer: "房间数",
      wohnflaeche: "居住面积",
      plz: "邮编",
      ort: "城市",
      stockwerk: "楼层",
      baujahr: "建造年份",
      balkon_terrasse: "阳台/露台",
      einbaukueche: "整体厨房",
      stellplatz: "车位",
      keller: "地下室",
      barrierefrei: "无障碍",
      heizungsart: "供暖方式",
      energieausweistyp: "能源证书",
      energietraeger: "能源类型",
      endenergiebedarf: "终端能耗需求",
      energieeffizienzklasse: "能效等级",
      hausgeld: "物业费",
      provision_kaeufer_prozent: "买方佣金",
      kaufnebenkosten: "购房附加费用",
      gesamtkosten: "总费用",
      kaltmiete: "净租金",
      nebenkosten_miete: "杂费",
      objektbeschreibung: "房产描述",
      lagebeschreibung: "位置",
      nutzflaeche: "使用面积",
      strasse: "街道",
      hausnummer: "门牌号",
      zustand: "房屋状况",
      wohneinheiten: "楼内单元数",
      vermietet: "是否出租",
      vermietet_seit: "出租起始日",
      stellplatz_anzahl: "车位数量",
      baujahr_waermeerzeuger: "供暖设备年份",
      hausgeld_nicht_umlagefaehig: "不可转嫁物业费",
      ruecklage_monatlich: "维修储备金",
    },
  },

  hi: {
    attachAria: "लिस्टिंग या स्क्रीनशॉट अपलोड करें",
    chipUpload: "📄 एक्सपोज़े अपलोड करें",
    dateiWaehlen: "लिस्टिंग या स्क्रीनशॉट चुनें",
    entfernenAria: "छवि हटाएँ",
    bilderGewaehlt: "{n} फ़ाइल चुनी गई",
    auswertenBtn: "लिस्टिंग पढ़ें",
    consentText:
      "विश्लेषण के लिए आपकी छवियाँ और PDF कुछ समय के लिए हमारे सर्वर पर भेजी जाती हैं और स्थायी रूप से संग्रहित नहीं की जातीं।",
    consentOk: "समझ गया",
    phaseUpload: "{m} में से {n} छवियाँ तैयार",
    phaseExtract: "लिस्टिंग पढ़ रहा हूँ",
    phaseExtractSchritte: ["संपत्ति डेटा", "लागत", "ऊर्जा", "स्थान"],
    resultTitel: "{gesamt} में से {gefunden} फ़ील्ड मिले · {pruefen} जाँचने हैं",
    roentgenTitel: "एक्सपोज़े एक्स-रे: जोखिम और खास बातें",
    risikoCode_sanierungsstau: "मरम्मत का बकाया",
    risikoCode_energieklasse_schlecht: "कमज़ोर ऊर्जा श्रेणी",
    risikoCode_modernisierung_angekuendigt: "आधुनिकीकरण की घोषणा",
    risikoCode_erbpacht: "लीज़होल्ड (Erbpacht)",
    risikoCode_denkmalschutz: "स्मारक संरक्षण",
    risikoCode_rechtsstreit: "कानूनी विवाद",
    risikoCode_miete_unter_markt: "बाज़ार से कम किराया",
    risikoCode_lange_standzeit: "लंबे समय से लिस्टेड",
    risikoCode_leerstand_lang: "लंबे समय से खाली",
    risikoCode_sonstiges: "अन्य ध्यान देने योग्य बात",
    gruppeObjekt: "संपत्ति",
    gruppeAusstattung: "सुविधाएँ",
    gruppeEnergie: "ऊर्जा",
    gruppeKosten: "लागत",
    gruppeKontext: "विवरण",
    uebernehmen: "लागू करें",
    allesMarkieren: "सभी चुनें",
    auswahlAufheben: "चयन हटाएँ",
    uebernommen: "{n} मान लागू किए गए",
    zumRechnerBtn: "कैलकुलेटर पर जाएं ▸",
    keineUebernahme: "कुछ नहीं चुना गया",
    aktuell: "वर्तमान: {wert}",
    nurInfo: "केवल जानकारी",
    nichtGefunden: "नहीं मिला",
    ja: "हाँ",
    nein: "नहीं",
    statusSicher: "भरोसेमंद ढंग से पढ़ा गया",
    statusUnsicher: "अनिश्चित — कृपया जाँचें",
    statusPruefen: "कृपया जाँचें",
    statusNichtGefunden: "नहीं मिला",
    fehlerDienst: "मैं अभी लिस्टिंग का विश्लेषण नहीं कर सका। कृपया थोड़ी देर बाद फिर कोशिश करें।",
    fehlerUnlesbar:
      "लिस्टिंग से कुछ भी नहीं पढ़ा जा सका। फ़ोटो के लिए अधिक रिज़ॉल्यूशन मदद करता है; PDF के लिए स्कैन किए पन्नों के बजाय असली टेक्स्ट वाली फ़ाइल दें।",
    fehlerLimit: "आपका लिस्टिंग कोटा फ़िलहाल समाप्त — Free में हर महीने की 1 तारीख को, Pro में कल नए मिलते हैं।",
    fehlerZugang: "लिस्टिंग स्कैन के लिए खाता चाहिए — सात दिन मुफ़्त।",
    fehlerDeaktiviert: "लिस्टिंग विश्लेषण फ़िलहाल उपलब्ध नहीं है।",
    fehlerOffline: "इसके लिए इंटरनेट चाहिए।",
    fehlerZuVieleBilder: "प्रति लिस्टिंग अधिकतम 15 छवियाँ।",
    fehlerPdfZuGross: "PDF बहुत बड़ा है (अधिकतम 25 MB)।",
    fehlerPdfZuVieleSeiten: "PDF में बहुत अधिक पृष्ठ हैं (अधिकतम 20)।",
    fehlerNurEinPdf: "प्रति लिस्टिंग केवल एक PDF।",
    fehlerTypNichtErlaubt: "केवल छवियाँ (JPG, PNG, WebP) या एक PDF।",
    fehlerZuGrossGesamt: "चयन कुल मिलाकर बहुत बड़ा है। कृपया कम छवियाँ लें।",
    fehlerLesen: "एक फ़ाइल पढ़ी नहीं जा सकी।",
    disclaimer:
      "ये मान विक्रेता की लिस्टिंग से हैं — असत्यापित। रहने का क्षेत्रफल और ऊर्जा आँकड़े व्यवहार में अक्सर भिन्न होते हैं; गंभीर रुचि होने पर मौके पर माप लें और मूल ऊर्जा प्रमाणपत्र देखें।",
    finnSagt: {
      upload:
        "ठीक है, मैं देखता हूँ। मैं लिस्टिंग पढ़कर वे मान निकालूँगा जो आपके कैलकुलेटर को चाहिए — क्रय मूल्य, रहने का क्षेत्रफल, रखरखाव शुल्क, ऊर्जा आँकड़े। टाइप करने की मेहनत बच जाएगी।",
      handout:
        "हो गया। मैंने लिस्टिंग को मिलान करके भी पढ़ा और {n} ऐसे बिंदु मिले जिन्हें हामी भरने से पहले स्पष्ट कर लेना चाहिए। आपका मुआयना हैंडआउट नीचे है: असली कीमत, खुले बिंदु और वे प्रश्न जो मौके पर सचमुच मायने रखते हैं। प्रिंट करें, साथ ले जाएँ, टिक करते जाएँ।",
      handoutLeer:
        "हो गया। मिलान पढ़ने पर कुछ बड़ा सामने नहीं आया — अच्छा संकेत। फिर भी मैंने मुआयने के लिए हैंडआउट तैयार किया है: एक नज़र में असली कीमत और वे प्रश्न जो लिस्टिंग अनुत्तरित छोड़ती है। इससे आप तैयार होकर जाएँगे।",
    },
    handout: {
      titel: "मुआयना हैंडआउट",
      oeffnen: "हैंडआउट खोलें",
      schliessen: "हैंडआउट बंद करें",
      groessterFund: "सबसे बड़ा निष्कर्ष {betrag}",
      keinFund: "कुछ असामान्य नहीं",
      automatisiert: "स्वचालित रूप से पहचाना गया — मोलभाव से पहले स्वयं जाँच लें।",
      findings: "मुख्य बिंदु",
      weitere: "+{n} और नहीं दिखाए गए",
      quellen: "कहाँ मिला: {quellen}",
      schwereCritical: "गंभीर",
      schwereWarning: "जाँचें",
      schwereInfo: "जानकारी",
      preis: "असली कीमत",
      beworben: "विज्ञापित",
      real: "वास्तविक",
      bekannt: "पहले से ज्ञात",
      offen: "अभी स्पष्ट करना है",
      vorOrt: "मौके पर जाँचें",
      leer: "कोई बड़ी बात सामने नहीं आई। नीचे दिए खुले प्रश्न फिर भी मुआयने में पूछने लायक हैं।",
      alleWaehlen: "सभी चुनें",
      alleAbwaehlen: "चयन हटाएँ",
      pdfBtn: "मुआयने के लिए PDF",
      pdfLeer: "कृपया कम से कम एक प्रश्न चुनें।",
      pdfDisclaimer: "यह कानूनी या निवेश सलाह नहीं है",
      pdfFooter: "Finn से बनाया गया · immofuchs.info",
      pdfVerdict: "Finn आकलन {score} / 5",
    },
    felder: {
      titel: "शीर्षक",
      objektart: "संपत्ति प्रकार",
      kaufpreis: "क्रय मूल्य",
      stellplatz_kaufpreis: "पार्किंग का मूल्य",
      kaufpreis_pro_qm: "प्रति m² मूल्य",
      zimmer: "कमरे",
      wohnflaeche: "रहने का क्षेत्रफल",
      plz: "पिन कोड",
      ort: "शहर",
      stockwerk: "मंज़िल",
      baujahr: "निर्माण वर्ष",
      balkon_terrasse: "बालकनी/छत",
      einbaukueche: "सुसज्जित रसोई",
      stellplatz: "पार्किंग",
      keller: "तहख़ाना",
      barrierefrei: "बाधामुक्त",
      heizungsart: "हीटिंग प्रकार",
      energieausweistyp: "ऊर्जा प्रमाणपत्र",
      energietraeger: "ऊर्जा स्रोत",
      endenergiebedarf: "अंतिम ऊर्जा माँग",
      energieeffizienzklasse: "दक्षता श्रेणी",
      hausgeld: "रखरखाव शुल्क",
      provision_kaeufer_prozent: "क्रेता कमीशन",
      kaufnebenkosten: "अतिरिक्त क्रय लागत",
      gesamtkosten: "कुल लागत",
      kaltmiete: "मूल किराया",
      nebenkosten_miete: "अन्य शुल्क",
      objektbeschreibung: "संपत्ति विवरण",
      lagebeschreibung: "स्थान",
      nutzflaeche: "उपयोग योग्य क्षेत्र",
      strasse: "सड़क",
      hausnummer: "मकान नंबर",
      zustand: "स्थिति",
      wohneinheiten: "भवन में इकाइयाँ",
      vermietet: "किराये पर",
      vermietet_seit: "किराये पर से",
      stellplatz_anzahl: "पार्किंग स्थान",
      baujahr_waermeerzeuger: "हीटिंग सिस्टम वर्ष",
      hausgeld_nicht_umlagefaehig: "गैर-वसूली योग्य मेंटेनेंस",
      ruecklage_monatlich: "रखरखाव आरक्षित निधि",
    },
  },
};

// Kleiner Platzhalter-Ersetzer, gleiches Muster wie in finnHints.js.
export function fuelle(vorlage, werte) {
  return Object.entries(werte).reduce(
    (text, [k, v]) => text.replace(new RegExp(`\\{${k}\\}`, "g"), String(v)),
    vorlage ?? "",
  );
}

// Loest einen gepunkteten Textschluessel ("finnSagt.upload") gegen den
// Sprachblock auf. Chat-Nachrichten von Finn werden als Schluessel abgelegt,
// nicht als fertiger Text - sonst bliebe eine Blase nach einem Sprachwechsel
// in der Sprache stehen, in der sie entstanden ist.
// Unbekannter Schluessel liefert null, damit die Blase leer bleibt statt
// "undefined" anzuzeigen.
export function loeseTextKey(key, t) {
  if (typeof key !== "string") return null;
  const wert = key.split(".").reduce((o, teil) => (o == null ? o : o[teil]), t);
  return typeof wert === "string" ? wert : null;
}

// Finns Einordnung ueber dem Handout. Zwei Fassungen, weil "ich habe 0 Punkte
// gefunden" niemand lesen will - und weil ein unauffaelliges Expose eine
// andere Nachricht verdient als eines mit drei kritischen Funden.
// Gezaehlt werden ALLE Findings, nicht nur die angezeigten fuenf.
export function handoutAnsage(analyse, t) {
  const anzahl = analyse?.findingsGesamt ?? 0;
  if (anzahl === 0) return t.finnSagt.handoutLeer;
  return fuelle(t.finnSagt.handout, { n: anzahl });
}
