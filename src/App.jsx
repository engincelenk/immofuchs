...
import { MARKET_RATES, GREST, BL_N, BL_O, KAPP15, MIET_P, SAN_ENERGIE, SAN_NORMEN, SAN_TIERS, SAN_SRC_KEYS, LAND_F, LAND_BONUS_FQ, LAND_BONUS_CAP } from "./data.js";
...
function Sanier(){
  ...
  return <div><VT view={view} setView={setView}/><div className="split">
    <div className={`inp-pane ${view==="input"?"act":""}`}>
      <Sec title={t.oL} icon="📍"/>
      <Sel label={t.bundesland} value={d.bundesland} onChange={v=>set("bundesland",v)} options={BL_O}/>
      {d.bundesland&&<div style={{fontSize:10,color:"var(--ch)",marginTop:-6,marginBottom:10,paddingLeft:4}}>🏦 Landesbank: {LAND_F[d.bundesland]||"BEG"} — BAFA-Fördersätze gelten bundesweit einheitlich</div>}
      <button onClick={()=>set("sanIsfp",!d.sanIsfp)} style={{display:"flex",alignItems:"center",gap:8,width:"100%",background:d.sanIsfp?"#dcfce7":"var(--ci)",border:`1px solid ${d.sanIsfp?"#22c55e":"var(--cb)"}`,borderRadius:8,padding:"8px 10px",cursor:"pointer",marginBottom:10,textAlign:"left",fontFamily:"inherit"}}>
        ...
        <div>
          <div style={{fontSize:12,fontWeight:600,color:d.sanIsfp?"#15803d":"var(--ct)"}}>iSFP — Individueller Sanierungsfahrplan</div>
          <div style={{fontSize:10,color:"var(--ch)",marginTop:1}}>+5% BAFA-Bonus auf alle BEG-Maßnahmen (Bundesförderung, § 89 GEG)</div>
        </div>
      </button>
    ...
    <div className={`res-pane ${view==="result"?"act":""}`}>
      ...
      {R.rows.length>0&&<div style={{background:"var(--cc)",borderRadius:12,padding:"12px",border:"1px solid var(--cb)",marginBottom:12}}>
        ...
        {R.tFoLand>0&&<div style={{fontSize:9,color:"#3b82f6",marginTop:4,paddingTop:4,borderTop:"1px solid var(--cb)"}}>* Landesbonus ({LAND_F[d.bundesland]}) — Schätzwert, bitte aktuell prüfen</div>}
      </div>}
      ...
      {d.sanIsfp&&<div style={{display:"flex",alignItems:"center",gap:6,background:"#dcfce7",border:"1px solid #86efac",borderRadius:8,padding:"6px 10px",marginBottom:10,fontSize:11}}>
        ...
        <span style={{fontWeight:600,color:"#15803d"}}>iSFP aktiv</span>
        <span style={{color:"#166534"}}>— +5% BAFA-Bonus auf alle BEG-Maßnahmen eingerechnet</span>
      </div>}
      ...
      <div style={{marginBottom:12}}>
        ...
        {d.sanIsfp
          ?<Ins emoji="✅" text="iSFP aktiviert — +5% BAFA-Bonus bereits eingerechnet. Beratungskosten 50% bezuschusst." type="good"/>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:18}}>
          {[
            ...
            {tab:"steuer6",title:l.st6Title,badge:l.st6Badge,desc:l.st6Desc,feats:[l.st6F1,l.st6F2,l.st6F3,l.st6F4,l.st6F5,l.st6F6],cta:l.st6Cta,
             ...
             illus:<svg viewBox="0 0 280 130" style={{width:"100%",height:"100%"}} preserveAspectRatio="xMidYMid slice"><text x="30" y="108" fontSize="90" fontWeight="800" fill="#1e3a5f" opacity=".09" fontFamily="DM Sans" letterSpacing="-4">§6</text><line x1="90" y1="30" x2="190" y2="100" stroke="#1e3a5f" strokeWidth="2" opacity=".35"/><circle cx="90" cy="38" r="18" fill="none" stroke="#e8600a" strokeWidth="2.5"/><circle cx="95" cy="38" r="6" fill="#e8600a" opacity=".6"/><circle cx="190" cy="92" r="18" fill="none" stroke="#e8600a" strokeWidth="2.5"/><circle cx="195" cy="92" r="6" fill="#e8600a" opacity=".6"/><rect x="155" y="22" width="95" height="26" rx="13" fill="#e8600a"/><text x="168" y="39" fontSize="12" fontWeight="700" fill="#fff" fontFamily="DM Sans">− Steuerlast</text><path d="M220 68 L220 88 L240 78 Z" fill="#1e3a5f" opacity=".45"/></svg>}
