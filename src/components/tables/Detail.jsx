...
export function Detail({R,d}){
  const{t}=useApp();
  const ek=+d.eigenkapital||0,sonder=+d.sonder||0,ren=+d.renovierung||0,nbk=R.nbk||0;
  const vw=R.vw;
  ...
  const row={display:"flex",justifyContent:"space-between",alignItems:"baseline",padding:"9px 14px",fontSize:12,borderBottom:"1px solid var(--cb)"};
  ...
  return <div style={{marginBottom:12}}>
    ...
    <div style={{background:"var(--cc)",border:"1px solid var(--cb)",borderRadius:10,overflow:"hidden"}}>
      <div style={row}>
        <div>
        ...
        <span style={{fontWeight:600,color:"var(--ct)",whiteSpace:"nowrap"}}>{fmtE(vw)}</span>
      </div>
      <div style={row}>
      ...
      <div style={row}>
        ...
        <span style={{fontWeight:600,color:"var(--ca)"}}>{R.sCFOhne>=0?"+":""}{fmtE(R.sCFOhne||0)}</span>
      </div>
      <div style={{padding:"14px",background:isPosOhne?"rgba(34,197,94,.08)":"rgba(239,68,68,.08)",borderTop:`2px solid ${isPosOhne?"#22c55e":"#ef4444"}`,display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,flexWrap:"wrap"}}>
        <span style={{fontSize:11,fontWeight:700,letterSpacing:.5,color:"var(--ct)"}}>{t.gSaldoOhne}</span>
        <span style={{textAlign:"right"}}>
          <div style={{fontSize:19,fontWeight:800,color:isPosOhne?"#15803d":"#b91c1c",fontVariantNumeric:"tabular-nums"}}>{isPosOhne?"+":""}{fmtE(totalOhne)}</div>
          <div style={{fontSize:10,color:"var(--ch)",marginTop:1}}>{t.detEKR}: {fmtP(rendEKOhne)} ({fmtP(ek>0?rendEKOhne/R.j:0)} p.a.)</div>
        </span>
      </div>
      ...
      <div style={{padding:"14px",background:isPosMit?"rgba(34,197,94,.08)":"rgba(239,68,68,.08)",borderTop:`2px solid ${isPosMit?"#22c55e":"#ef4444"}`,display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,flexWrap:"wrap"}}>
        <span style={{fontSize:11,fontWeight:700,letterSpacing:.5,color:"var(--ct)"}}>{t.gSaldoMit}</span>
        <span style={{textAlign:"right"}}>
          <div style={{fontSize:19,fontWeight:800,color:isPosMit?"#15803d":"#b91c1c",fontVariantNumeric:"tabular-nums"}}>{isPosMit?"+":""}{fmtE(totalMit)}</div>
          <div style={{fontSize:10,color:"var(--ch)",marginTop:1}}>{t.detEKR}: {fmtP(rendEKMit)} ({fmtP(ek>0?rendEKMit/R.j:0)} p.a.)</div>
        </span>
      </div>
    </div>
    <div style={{fontSize:9,color:"var(--ch)",marginTop:6,lineHeight:1.4}}>{t.detSteuerHinweis}</div>
  </div>;
}
