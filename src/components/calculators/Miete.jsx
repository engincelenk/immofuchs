            ...
            <>
              ...
              >
                ...
                <div style={{ fontSize: 12, lineHeight: 1.8 }}>
                  {[
                    ...
                    [
                      t.kapp,
                      <span style={{ fontWeight: 600, color: "var(--ca)" }}>{R.kP}% in 3 J.</span>,
                    ],
                    [t.markt, R.k15 ? "🔴 " + t.ang : "🟢 " + t.std],
                    ...(R.vm ? [[t.vgl, fmtE(R.vm) + "/Mon."]] : []),
                  ].map(([k, v], i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ color: "var(--ch)" }}>{k}</span>
                      <span>{v}</span>
                    </div>
                  ))}
                </div>
                        ...
                        {[
                          [
                            ...
                            <span style={{ fontWeight: 600, color: "var(--ca)" }}>
                              {fmtDat(nx.datum, lang)}
                            </span>,
                          ],
                          [t.akt, fmtE(nx.aktMiete) + "/Mon."],
                          [
                            t.mxE,
              ...
              {R.rows.length > 0 && (
                ...
                >
                  ...
                  <table style={{ width: "100%", fontSize: 11, borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ borderBottom: "1px solid var(--cb)" }}>
                        {[t.dat, t.akt, t.vgl, t.erh, t.nM, t.sta].map((h) => (
                          <th
                            key={h}
                            style={{
                              padding: "3px 4px",
                    ...
                    <tbody>
                      {R.rows.map((r, i) => (
                        <tr key={i} style={{ borderBottom: "1px solid var(--cb)" }}>
                          ...
                          <td
                            style={{ padding: "3px 4px", textAlign: "right", color: "var(--ch)" }}
                          >
                            {r.vmProg ? fmtE(Math.round(r.vmProg)) : "—"}
                          </td>
                          <td
                            style={{
                              padding: "3px 4px",
                              textAlign: "right",
                              color: r.mE > 0 ? "#22c55e" : "var(--ch)",
                            }}
                          >
                            {r.mE > 0 ? `+${fmtE(r.mE)}` : "—"}
                          </td>
                          <td
                            style={{ padding: "3px 4px", textAlign: "right", color: "var(--ca)" }}
                          >
                            {fmtE(r.neueMiete)}
                          </td>
                          <td
                  ...
                  <div style={{ fontSize: 10, color: "var(--ch)", marginTop: 6 }}>
                    {R.vQ > 0 ? `📊 Ø +${fmt(R.vmPA, 1)}% p.a. | ${R.q}` : ""}
                  </div>
                </div>
              )}
              <div style={{ marginBottom: 12 }}>
                ...
                {(() => {
                  ...
                  const jahre3 = lD ? (jetzt - lD) / (1000 * 60 * 60 * 24 * 365.25) : 99;
                  return (
                    <>
                      {vm > 0 && mi > 0 && mi < vm * 0.85 && nx && nx.verfK > 0 && (
                        <Ins emoji="📈" text={t.adv12} type="info" />
                      )}
                      {jahre3 > 3 && <Ins emoji="🔄" text={t.adv13} type="good" />}
                      {nx && nx.verfK <= 0 && vm > 0 && mi < vm * 0.95 && (
                        <Ins emoji="⏳" text={t.adv14} type="warn" />
                      )}
                      {R.k15 && vm > 0 && mi >= vm * 0.9 && (
                        <Ins emoji="🔧" text={t.adv15} type="info" />
                      )}
                    </>
                  );
                })()}
