...
export function LineChart({ rows, zbJ }) {
  ...
  const rA = rows.map((r) => r.rest),
    cA = rows.map((r) => r.cfKum),
    mA = rows.map((r) => r.miete);
  const mxR = Math.max(...rA, ...mA, 1); // left axis includes both Restschuld and Jahresmiete
  const all = [...cA, 0];
  const mnS = Math.min(...all),
    mxS = Math.max(...all),
    rS = mxS - mnS || 1;
      ...
      <div style={{ position: "relative", overflowX: "auto" }}>
        ...
        {hover !== null && rows[hover] && (
          ...
          >
            ...
            >
              J{rows[hover].j}
              {zbIdx !== null && rows[hover].j === zbJ ? " ◀ ZB" : ""}
            </div>
            <div style={{ color: "#ef8888" }}>
              {t.chartRestschuld}: {fmtE(rows[hover].rest)}
            </div>
            <div style={{ color: "#ef8888" }}>
              {t.gZin}: {fmtE(rows[hover].zinsen)}
            </div>
            <div style={{ color: "#6ddb8a" }}>
              {t.steuerErs}: {fmtE(rows[hover].steuer)}
            </div>
            <div style={{ color: "#ffa64d" }}>
              {t.chartHoverJahresmiete}: {fmtE(rows[hover].miete)}
            </div>
            <div
              style={{
                color: (rows[hover].cfOhneSt ?? 0) >= 0 ? "#ffa64d" : "#ef8888",
                marginTop: 2,
              }}
            >
              {t.chartHoverCFOhne}: {fmtE(rows[hover].cfOhneSt ?? 0)}
            </div>
            <div style={{ color: rows[hover].cf >= 0 ? "#6ddb8a" : "#ef8888" }}>
              {t.chartHoverCFMit}: {fmtE(rows[hover].cf)}
            </div>
            <div
              style={{
                color: rows[hover].cfKum >= 0 ? "#6ddb8a" : "#ef8888",
                borderTop: "1px solid #444",
                paddingTop: 3,
                marginTop: 3,
              }}
            >
              {t.chartHoverKumCF}: {fmtE(rows[hover].cfKum)}
            </div>
          </div>
        )}
      </div>
          ...
          >
            ...
            {rows.map((r, i) => (
              <rect
                ...
                height={ph}
                fill="transparent"
                onMouseEnter={() => setHoverCF(i)}
                onMouseLeave={() => setHoverCF(null)}
                style={{ cursor: "crosshair" }}
              />
            ))}
          </svg>
          {hoverCF !== null && rows[hoverCF] && (
            <div
              style={{
                position: "absolute",
                top: 0,
                left: xS(hoverCF) > W / 2 ? "auto" : "calc(" + (xS(hoverCF) * 100) / W + "% + 8px)",
                right:
                  xS(hoverCF) > W / 2
                    ? "calc(" + (100 - (xS(hoverCF) * 100) / W) + "% + 8px)"
                    : "auto",
                background: "#1a1a1a",
                color: "#fff",
                borderRadius: 8,
            ...
            >
              ...
              <div style={{ color: "#ffa64d" }}>
                {t.cfOhneSt}: {fmtE(cfOhneArr[hoverCF])}
              </div>
              <div style={{ color: "#6ddb8a" }}>
                {t.cfMitSt}: {fmtE(cfMitArr[hoverCF])}
              </div>
              <div style={{ color: "#aaa", marginTop: 2 }}>
                {t.chartHoverSteuervorteil}: {fmtE(cfMitArr[hoverCF] - cfOhneArr[hoverCF])}
              </div>
            </div>
          )}
        </div>
        <div style={{ fontSize: 10, color: "var(--ch)", marginTop: 6, fontStyle: "italic" }}>
          {t.chartDisclamer}
        </div>
      </div>
    </div>
