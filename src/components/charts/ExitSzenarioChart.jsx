...
export function ExitSzenarioChart({ R, wertP, sonder, renovierung, altAnlage, eigenkapital }) {
  ...
  rows.forEach((r, idx) => {
    ...
    const gesamtImmo =
      wertsteigKum + tilgKum + cfKum - (R.nbk || 0) - (+sonder || 0) - (+renovierung || 0);
  ...
  const milestoneSet = [Math.ceil(n * 0.25), Math.ceil(n * 0.5), Math.ceil(n * 0.75), n].filter(
  ...
  return (
    ...
    >
      <div style={{ fontSize: 12, fontWeight: 700, color: "var(--ct)", marginBottom: 2 }}>
        {t.chartExitTitle || "Netto-Ergebnis je Ausstiegsjahr"}
      </div>
      <div style={{ fontSize: 10.5, color: "var(--ch)", marginBottom: 8 }}>
        {t.chartExitSub || "Immobilie vs. Alternativanlage je Ausstiegsjahr"}
      </div>
      <div style={{ position: "relative", overflowX: "auto" }}>
        ...
        >
          ...
          {vals.map((v, i) => {
            ...
            const immoY = v.gesamtImmo >= 0 ? yv(v.gesamtImmo) : y0,
              immoH = Math.abs(yv(v.gesamtImmo) - y0);
            const altY = v.gesamtAlt >= 0 ? yv(v.gesamtAlt) : y0,
              altH = Math.abs(yv(v.gesamtAlt) - y0);
            return (
              <g
                key={i}
      ...
      >
        ...
        <span>
          ...
          />
          {t.chartAltAnlage || "Alternativanlage"} ({altAnlage || 6.5}%)
        </span>
      </div>
      {hover !== null && vals[hover] && (
        <div style={{ fontSize: 10.5, color: "var(--ct)", marginTop: 8, lineHeight: 1.6 }}>
          J{vals[hover].j}: {t.chartImmobilie || "Immobilie"} <b>{fmtE(vals[hover].gesamtImmo)}</b>{" "}
          · {t.chartAltAnlage || "Alternativanlage"} <b>{fmtE(vals[hover].gesamtAlt)}</b>
        </div>
      )}
      <div
        style={{
      ...
      >
        {beJahr
          ? (t.chartExitBreakEven || "Break-even Gesamtposition: ab Jahr {j}").replace(
              "{j}",
              beJahr,
            )
          : t.chartDisclamer || ""}
