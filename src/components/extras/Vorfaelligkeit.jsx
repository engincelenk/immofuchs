...
export function Vorfaelligkeit() {
  ...
  const R = useMemo(() => {
    ...
    while (cur < ablT) {
      ...
      if (sondJ > 0 && y > lastSondYear) {
        rs = Math.max(0, rs - sondJ);
        lastSondYear = y;
      }
      const zi = rs * mz,
        ti = Math.max(0, effRate - zi);
      rs = Math.max(0, rs - ti);
    ...
    while (rsL > 0.01 && curL < horizon) {
      ...
      if (sondJ > 0 && yS > lastSondYear) {
        rsL = Math.max(0, rsL - sondJ);
        lastSondYear = yS;
      }
      const zV = rsL * mz,
        zWA = rsL * waMon;
      ...
      yrZinsWA += zWA;
      yrTilg += ti;
      yrZinsverlust += zinsverlust;
      yrAbg += zinsverlustAbg;
    ...
    return {
      is489Free: false,
      rsKuend,
      zinsverschlSchaden: totalZinsV,
      risikoersparnis: totalRisiko,
      verwaltungsersparnis: totalVerwaltung,
      bearbeitungsentgelt: bearbeit,
      nettovfe,
      rows,
      months: month,
      effZP,
    ...
    <div>
      ...
      <div className="split">
        <div className={`inp-pane ${view === "input" ? "act" : ""}`}>
          ...
          <Row>
            <F
              label={vt.lAuszahlung}
              type="date"
              value={d.vfeAuszahlung || ""}
              onChange={(v) => set("vfeAuszahlung", v)}
              tip={vt.tipAuszahlung}
            />
            <F
              label={vt.lZbEnde}
              type="date"
              value={d.vfeSollzinsbindungsEnde || ""}
              onChange={(v) => set("vfeSollzinsbindungsEnde", v)}
              tip={vt.tipZbEnde}
            />
          </Row>
          <Row>
            <F
              label={vt.lRestschuld}
              unit="€"
              value={d.vfeRestschuld || ""}
              onChange={(v) => set("vfeRestschuld", v)}
              placeholder={vt.phRestschuld}
              tip={vt.tipRestschuld}
            />
            <F
              label={vt.lRestschuldDatum}
              type="date"
              value={d.vfeRestschuldDatum || today2}
              onChange={(v) => set("vfeRestschuldDatum", v)}
              tip={vt.tipRestschuldDatum}
            />
          </Row>
          <Row>
            <F
              label={vt.lNominalzins}
              unit="%"
              value={effZinsDisp}
              onChange={(v) => set("vfeSollzinssatz", v)}
              step="0.01"
              tip={vt.tipNominalzins}
              hint={!d.vfeSollzinssatz && d.zinssatz ? vt.hintFin : ""}
            />
            <F
              label={vt.lRate}
              unit="€"
              value={effRateDisp}
              onChange={(v) => set("vfeMonatsRate", v)}
              step="1"
              tip={vt.tipRate}
              hint={!d.vfeMonatsRate && autoRate > 0 ? vt.hintCalc : ""}
            />
          </Row>
          <Sec title={vt.secKuend} icon="📋" />
          <F
            label={vt.lKuendigung}
            type="date"
            value={d.vfeAbloeseTermin || ""}
            onChange={(v) => set("vfeAbloeseTermin", v)}
            tip={vt.tipKuendigung}
          />
          <Row>
            <F
              label={vt.lSonderJ}
              unit="€"
              value={d.vfeSondertilgung || "0"}
              onChange={(v) => set("vfeSondertilgung", v)}
              tip={vt.tipSonderJ}
            />
            <F label={vt.lSonderGeleistet} tip={vt.tipSonderGeleistet}>
              <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                {["nein", "ja"].map((v) => (
                  <button
                    key={v}
                    onClick={() => set("vfeSondertilgungGeleistet", v)}
                    style={{
                      flex: 1,
                      padding: "10px",
                      borderRadius: 10,
                      border: `1px solid ${(d.vfeSondertilgungGeleistet || "nein") === v ? "var(--ca)" : "var(--cb)"}`,
                      background:
                        (d.vfeSondertilgungGeleistet || "nein") === v
                          ? "var(--ca-bg)"
                          : "var(--ci)",
                      color:
                        (d.vfeSondertilgungGeleistet || "nein") === v ? "var(--ca)" : "var(--cl)",
                      fontWeight: 600,
                      fontSize: 14,
                      cursor: "pointer",
                    }}
          ...
          <F
            label={vt.lWiederanlage}
            unit="%"
            value={d.vfeWiederanlagezins || String(PFANDBRIEF.zins)}
            onChange={(v) => set("vfeWiederanlagezins", v)}
            step="0.01"
            tip={vt.tipWiederanlage}
            hint={vt.hintPfand}
          />
          <F
            label={vt.lBearbeitung}
            unit="€"
            value={d.vfeBearbeitungsentgelt || "150"}
            onChange={(v) => set("vfeBearbeitungsentgelt", v)}
            tip={vt.tipBearbeitung}
          />
          <button
                  ...
                  >
                    ...
                    <tbody>
                      ...
                      <tr style={{ fontWeight: 700, background: "var(--cro)" }}>
                        ...
                        >
                          {fmtE(R.rows.reduce((a, r) => a + r.tilg, 0))}
                        </td>
                        <td
                          style={{
                            padding: "6px 10px",
                        ...
                        >
                          {fmtE(R.rows.reduce((a, r) => a + r.zinsV, 0))}
                        </td>
                        <td
                          style={{
                            padding: "6px 10px",
                        ...
                        >
                          {fmtE(R.rows.reduce((a, r) => a + r.zinsWA, 0))}
                        </td>
                        <td
                          style={{
                            padding: "6px 10px",
                        ...
                        >
                          {fmtE(R.rows.reduce((a, r) => a + r.zinsverlust, 0))}
                        </td>
                        <td
                          style={{
                            padding: "6px 10px",
                        ...
                        >
                          {fmtE(R.rows.reduce((a, r) => a + r.abgezinst, 0))}
                        </td>
                      </tr>
                    </tbody>
                  </table>
