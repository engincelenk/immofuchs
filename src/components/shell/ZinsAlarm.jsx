...
export function ZinsAlarm({ zinsen, lang }) {
  ...
  const [threshold, setThreshold] = useState(() => {
    try {
      return parseFloat(localStorage.getItem("if_alarm_threshold") || "3.5");
    } catch {
      return 3.5;
    }
  });
  const [enabled, setEnabled] = useState(() => {
    try {
      return localStorage.getItem("if_alarm_enabled") === "1";
    } catch {
      return false;
    }
  });
  ...
  return (
    <div style={card}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <input
            ...
            onChange={(e) => {
              ...
              if (!isNaN(v)) {
                setThreshold(v);
                if (enabled) persistAlarm(enabled, v);
              }
            }}
            style={{
              width: 76,
        ...
        {permission === "denied" ? (
          <span style={{ fontSize: 12, color: "#e74c3c", marginLeft: "auto" }}>
            {l.alarmDenied}
          </span>
        ) : (
