...
export function F({label,unit,value,onChange,type="number",step,readOnly,hint,tip,placeholder,children}){
...
export function Sel({label,value,onChange,options}){return <F label={label}><select value={value} onChange={e=>{const v=e.target.value;if(v!==String(value))onChange(v);}} style={{width:"100%",padding:"12px 14px",fontSize:18,border:"1px solid var(--cb)",borderRadius:10,background:"var(--ci)",color:"var(--ct)",fontFamily:"inherit",minHeight:46}}>{options.map(o=><option key={o.v} value={o.v}>{o.l}</option>)}</select></F>}
export function Row({children}){return <div className="if-row">{children}</div>}
export function Sec({title,icon}){return <div style={{display:"flex",alignItems:"center",gap:8,margin:"24px 0 14px",paddingBottom:8,borderBottom:"1px solid var(--cb)"}}><span style={{fontSize:18}}>{icon}</span><span style={{fontSize:16,fontWeight:600,color:"var(--ct)"}}>{title}</span></div>}
...
export function VT({view,setView}){const{t}=useApp();return <div className="mob-toggle">{["input","result"].map(v=><button key={v} className={view===v?"act":""} onClick={()=>{setView(v);setTimeout(()=>window.scrollTo({top:0,behavior:'smooth'}),50)}}>{v==="input"?t.eingabe:t.ergebnis}</button>)}</div>}
