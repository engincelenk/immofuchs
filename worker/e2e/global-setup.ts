...
// EINMAL frische Sessions per echtem Login und legt sie in
// `worker/e2e/.sessions.json` ab; setup.ts liest sie von dort.
//
// WARUM ueberhaupt: bis 2026-08-19 standen feste Session-IDs an drei Stellen
// (worker/e2e/run.ps1, e2e-dashboard/server.js als Fallback,
// e2e-dashboard/.env.local). Die drei Stellen sind auseinandergelaufen - in
// .env.local standen zuletzt zwei Paddle-PREIS-IDs (pri_...) statt
// Session-IDs, und weil server.js `process.env.X || Fallback` benutzt, haben
// die falschen Werte die funktionierenden ueberschrieben: 63 von 118 Tests
// rot, fast alle mit 401 not_authenticated. Sessions, die die Suite sich
// selbst holt, koennen weder ablaufen noch falsch abgetippt werden.
//
...
export async function setup(): Promise<void> {
  ...
  for (const account of ACCOUNTS) {
    ...
    if (!password) {
      const msg = `${account.passwordEnv} ist nicht gesetzt (siehe e2e-dashboard/.env.local).`;
    ...
    if (blockedUntil > now) {
      ...
      const msg =
        ...
        `nicht erneut versucht. Passwort in e2e-dashboard/.env.local pruefen.`;
      if (account.required) throw new Error(`[e2e] ${msg}`);
    ...
    if (!result.ok) {
      ...
      if (account.required) throw new Error(`[e2e] ${msg} Passwort in e2e-dashboard/.env.local pruefen.`);
      console.warn(`[e2e] ${msg} Admin-Tests ueberspringen sich selbst.`);
      continue;
    }
