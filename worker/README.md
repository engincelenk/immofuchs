...
3. ✅ **Kill-Switch:** `ASSISTANT_ENABLED` auf `false` setzen -> naechster Request liefert sofort `503`. Zurueck auf `true` -> normaler Betrieb. **Live bestaetigt** (2026-07-20, per `wrangler deploy`-Toggle statt Dashboard-Klick getestet, gleiche Codepfad-Logik). Fuer den echten "ohne Redeploy"-Weg im Betrieb: Cloudflare-Dashboard -> Workers & Pages -> `immofuchs-assistant` -> Settings -> Variables (nicht ueber `wrangler.toml`).
...
6. ✅ **Sprach-Routing:** Gleiche Frage mit `lang: "tr"` senden - sollte ueber Gemini laufen (`GEMINI_API_KEY` muss gesetzt sein), nicht ueber Workers AI. **Code bestaetigt korrekt** (2026-07-20) — Modellname war urspruenglich falsch (`gemini-2.5-flash-lite` existiert fuer den Key nicht), korrigiert auf `gemini-2.0-flash-lite`; Auth per `x-goog-api-key`-Header (neuerer Google-Key-Typ). Fehler wechselte dadurch von `404` zu `429` (Googles eigenes Tageskontingent durch Testanfragen ausgeschoepft) - reine Kontingentfrage, kein Code-Bug mehr. Volle End-to-End-Antwort steht noch aus, bis das Google-Kontingent sich erholt.
