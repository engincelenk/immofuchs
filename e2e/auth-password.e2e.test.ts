...
describe("POST /auth/register", () => {
  // 429-Sonderfall (2026-08-19): registerWithPassword() prueft das
  // Stundenlimit `register:<email>` (REGISTER_HOURLY_LIMIT = 5, siehe
  // worker/src/auth/passwordAuth.ts) VOR der Pruefung auf eine bereits
  // vergebene E-Mail. Dieser Test verbraucht bei JEDEM Lauf einen der fuenf
  // Versuche fuer test.monatlich - ab dem 6. Lauf innerhalb einer Stunde
  // antwortet der Endpunkt also voellig korrekt mit 429, bevor er ueberhaupt
  // zum 409 kommt.
  //
  // Genau so passiert: am 19.08. lief die Suite beim Ordner-Umzug 7x in 45
  // Minuten, der Test wurde daraufhin rot - ohne dass sich am Produkt etwas
  // geaendert haette. Ein Test, der vom Zaehlerstand frueherer Laeufe
  // abhaengt, ist prinzipbedingt instabil.
  //
  // Deshalb: 429 ist KEIN Fehlschlag, sondern das erwartete Verhalten eines
  // ausgeschoepften Limits - der Fall wird dann uebersprungen statt rot zu
  // faerben. Das eigentlich zu pruefende Verhalten (409 email_taken mit
  // providers) wird unveraendert streng geprueft, sobald wieder Budget da
  ...
  it("bereits vergebene E-Mail (test.monatlich) -> 409 email_taken mit providers", async (ctx) => {
    ...
    if (res.status === 429) {
      ctx.skip(
        `Stundenlimit fuer register:${credentials.monatlich.email} ausgeschoepft ` +
          `(${"REGISTER_HOURLY_LIMIT"} = 5/Stunde). Die Suite lief zu oft innerhalb einer Stunde - ` +
          `in ca. einer Stunde laeuft dieser Fall wieder mit. Kein Produktfehler.`,
      );
      return;
    }
    if (res.status === 403) {
