...
// (e2e/last-report.html, nicht committet - siehe .gitignore). Nutzt bewusst
// nur den in Vitest eingebauten JSON-Reporter statt eines zusaetzlichen
// UI-Pakets (@vitest/ui) - eine Abhaengigkeit weniger fuer dieses kleine
// QA-Skript.
//
// Ordner-Umzug 2026-08-19 (Nutzerwunsch "alles in einem Ordner"): diese
// Datei lag vorher unter worker/e2e/generate-report.mjs und wurde ueber
// worker/package.json (`npm run test:e2e:report`, cwd=worker/) gestartet.
// Jetzt liegt sie im flachen e2e/-Ordner direkt unter dem Projekt-Root und
// wird ueber das ROOT-package.json gestartet (cwd=Projekt-Root) - deshalb
// bekommt der vitest-Aufruf unten den Praefix "e2e/" fuer die Config, und
// process.cwd() + "e2e" + Dateiname trifft weiterhin genau diesen Ordner
// (er heisst ja "e2e" und liegt jetzt direkt unter dem Root).
...
const run = spawnSync(
  "npx",
  ["vitest", "run", "--config", "e2e/vitest.e2e.config.ts", "--reporter=verbose", "--reporter=json", `--outputFile.json=${jsonPath}`],
