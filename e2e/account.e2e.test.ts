...
// GET /me (siehe me.e2e.test.ts). test.monatlich ist eine geteilte,
// dauerhafte Fixture (siehe README.md) - jede hier getestete Aenderung
// (Name, Marketing-Mails) wird am Ende wieder auf den urspruenglichen Wert
// zurueckgesetzt (beforeAll erfasst den Ist-Zustand), damit ein Testlauf
// keine Spuren im Fixture-Konto hinterlaesst.
// (Bis 2026-08-18 lief dieser Test ueber test.free - Konto wurde geloescht
// und wird nicht mehr verwendet, siehe release-notes.txt.)
//
...
describe("Account-Routen (test.monatlich)", () => {
  ...
  afterAll(async () => {
    await apiFetch(sessions.monatlich(), "/api/v1/account/name", {
      method: "POST",
      body: JSON.stringify({ name: originalName || "Test Monatlich" }),
    });
    await apiFetch(sessions.monatlich(), "/api/v1/account/notifications", {
      method: "POST",
  ...
  it("GET /account/devices listet die aktuelle Session als current=true", async () => {
    const res = await apiFetch(sessions.monatlich(), "/api/v1/account/devices");
  ...
  it("POST /calculator-trial/consume mit gueltigem Rechner -> 200 ok", async () => {
    const res = await apiFetch(sessions.monatlich(), "/api/v1/calculator-trial/consume", {
      method: "POST",
  ...
  it("POST /account/name mit leerem Namen -> 400 invalid_name", async () => {
    const res = await apiFetch(sessions.monatlich(), "/api/v1/account/name", {
      method: "POST",
  ...
  it("POST /account/name aendert den Namen, GET /me spiegelt ihn danach", async () => {
    ...
    const me = await apiFetch(sessions.monatlich(), "/api/v1/me");
  ...
  it("POST /account/notifications toggelt marketingEmailsEnabled", async () => {
    ...
    const res = await apiFetch(sessions.monatlich(), "/api/v1/account/notifications", {
      method: "POST",
  ...
  it("POST /account/email mit ungueltigem Format -> 400 invalid_email", async () => {
    const res = await apiFetch(sessions.monatlich(), "/api/v1/account/email", {
      method: "POST",
  ...
  // von test.jaehrlich) liefert dieselbe generische {ok:true}-Antwort wie
  ...
  // test.monatlich bleibt unveraendert, hier ueber ein anschliessendes
  // GET /me verifiziert statt nur angenommen.
  it("POST /account/email auf eine bereits vergebene Adresse -> generisches {ok:true}, keine Aenderung", async () => {
    const res = await apiFetch(sessions.monatlich(), "/api/v1/account/email", {
      method: "POST",
      body: JSON.stringify({ newEmail: "test.jaehrlich@immofuchs.info" }),
    });
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ ok: true });

    const me = await apiFetch(sessions.monatlich(), "/api/v1/me");
    expect((await me.json()).email).toBe("test.monatlich@immofuchs.info");
  });

  it("GET /account/export liefert einen DSGVO-Datenexport mit Content-Disposition", async () => {
    const res = await apiFetch(sessions.monatlich(), "/api/v1/account/export");
    expect(res.status).toBe(200);
    expect(res.headers.get("Content-Disposition")).toContain("immofuchs-daten-export.json");
    const body = await res.json();
    expect(body.account.email).toBe("test.monatlich@immofuchs.info");
    expect(Array.isArray(body.sessions)).toBe(true);
  });
  ...
  // mitgeschickt). Welcher der beiden Faelle fuer test.monatlich zutrifft, ist
  ...
  it("POST /account/delete ohne (gueltiges) Passwort loescht nichts - Konto bleibt danach abrufbar", async () => {
    const res = await apiFetch(sessions.monatlich(), "/api/v1/account/delete", {
      method: "POST",
      body: JSON.stringify({}),
    });
